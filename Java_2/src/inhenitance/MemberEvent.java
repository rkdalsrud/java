...
//상속(Inheritance) : 클래스를 선언할 때 기존 클래스를 물려받아(필드 메소드) 사용하는 기능
...
/*public class MemberEvent extends Member {
	...
	private String email;
	//이메일을 멤버클래스에 저장하지 않은 이유는 MemberApp도 다시 고쳐야 되기 때문에 차라리 따로 클래스 생성
	...
	//위해 super 키워드 사용 - 부모클래스 객체가 생성될 때 필드에 원하는 초기값 저장 가능-매개변수가 있는 생성자
	//형식) super(값, 값, ...);
	...
	*/
	...
	public void display() {
		...
		System.out.println("이메일 = "+email);
	}//은닉화 처리되어 있다면 get나 supper 사용 가능(getter사용한 것)
	*/
	...
	/*@Override
	public void display() {
		super.display();//super 키워드로 부모클래스의 숨겨진 메소드 호출(setter가 아닌 supper사용한 것)
		System.out.println("이메일 = "+email);
	}
	*/
